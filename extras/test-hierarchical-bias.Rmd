...
fitModels <- function(trueRR, metaLPs, cachePath = "cache", seed = 666){
  
  res = list(RR = trueRR)
  
  # 1 ## fit a joint model with main exposure effect included (uninformative mean):
  resFile = file.path(cachePath, 
                      sprintf("fittedModel-uninformed-%s.rds", trueRR))
  ...
  resFile = file.path(cachePath, 
                      sprintf("fittedModel-informed-%s.rds", trueRR))
  if(file.exists(resFile)){
    maWithExposure = readRDS(resFile)
  }else{
  ...
  resFile = file.path(cachePath, 
                      sprintf("fittedModel-separate-%s.rds", trueRR))
  if(file.exists(resFile)){
    maWithExposure = readRDS(resFile)
  }else{
    settings = generateBayesianHMAsettings(globalExposureEffectPriorMean = c(log(trueRR)),
  ...
  resFile = file.path(cachePath, 
                      sprintf("fittedModel-twoStage-%s.rds", trueRR))
  if(file.exists(resFile)){
...
theCachePath = "cache-3"
if(!dir.exists(theCachePath)){dir.create(theCachePath)}
...
for(trueRR in trueRRs){
  ...
  this.res = fitModels(trueRR, metaLPs, cachePath = theCachePath)
