...
plotLikelihoodFit <- function(approximation,
  ...
  plot <- ggplot2::ggplot(plotData, ggplot2::aes(x = .data$x, y = .data$ll)) +
    ...
    ggplot2::geom_line(ggplot2::aes(group = .data$type, color = .data$type, linetype = .data$type, size = .data$type), alpha = 0.7) +
    ...
    ggplot2::scale_x_continuous(xLabel, limits = log(limits), breaks = log(breaks), labels = breaks) +
...
plotMcmcTrace <- function(estimate, showEstimate = TRUE, dataCutoff = 0.01, fileName = NULL) {
  ...
  plot <- with(data, {
    ggplot2::ggplot(data, ggplot2::aes(x = .data$x, y = .data$trace)) + 
  ...
  if (showEstimate) {
    mode <- data.frame(trace = c(estimate$mu, estimate$tau),
                       var = c("Mu", "Tau"))
    ci <- data.frame(trace = c(estimate$mu95Lb, estimate$mu95Ub, estimate$tau95Lb, estimate$tau95Ub),
                     var = c("Mu", "Mu", "Tau", "Tau"))
    
    plot <- plot + ggplot2::geom_hline(ggplot2::aes(yintercept = trace), data = mode) +
...
plotPerDbMcmcTrace <- function(estimate, showEstimate = TRUE, dataCutoff = 0.01, fileName = NULL) {
  ...
  if (showEstimate) {
    ...
    plot <- plot + ggplot2::geom_hline(ggplot2::aes(yintercept = .data$trace), data = mode) +
      ggplot2::geom_hline(ggplot2::aes(yintercept = .data$trace), data = ci, linetype = "dashed") 
  }
  if (!is.null(fileName))
    ggplot2::ggsave(fileName, plot, width = 6, height = 4.5, dpi = 400)
  return(plot)
...
plotPosterior <- function(estimate, showEstimate = TRUE, dataCutoff = 0.01, fileName = NULL) {
  ...
  data <- rbind(dataMu, dataTau)
  plot <- with(data, {
    ggplot2::ggplot(data, ggplot2::aes(x = .data$trace)) + 
  ...
  if (showEstimate) {
    ...
    plot <- plot + ggplot2::geom_vline(ggplot2::aes(xintercept = .data$trace), data = mode) +
      ggplot2::geom_vline(ggplot2::aes(xintercept = .data$trace), data = ci, linetype = "dashed") 
  }
  if (!is.null(fileName))
    ggplot2::ggsave(fileName, plot, width = 6, height = 4.5, dpi = 400)
  return(plot)
...
plotPerDbPosterior <- function(estimate, showEstimate = TRUE, dataCutoff = 0.01, fileName = NULL) {
  ...
  if (showEstimate) {
    ...
    plot <- plot + ggplot2::geom_vline(ggplot2::aes(xintercept = trace), data = mode) +
