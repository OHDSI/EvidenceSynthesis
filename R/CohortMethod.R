...
#'                   `computeCovariateBalance()` function in the `CohortMethod` package. Each balance object is expected to be a data frame with 
#'                   at least these two columns: `beforeMatchingStdDiff` and `afterMatchingStdDiff`.
...
#' 
#' @param preparedPsPlots    list of prepared propensity score data as created by the 
#'                          [preparePsPlot()] function.
#' @param labels            A vector containing the labels for the various sources.         
...
#'                          See the function [ggplot2::ggsave] for supported file
#'                          formats.
#'                          
#' @examples
#' treatment <- rep(0:1, each = 100)
